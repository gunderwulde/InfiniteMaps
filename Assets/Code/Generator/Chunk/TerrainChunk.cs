...
#region Heightmap stuff
        ...
        float GetNoise(float x, float y) {
            ...
            for (var i = 0; i < _octaveCount; i++) {
                ...
                float signal = Mathf.PerlinNoise(sx, sy) * 2 -1; 
            ...
            lock (HeightmapThreadLockObject) {
                ...
                for (var zRes = 0; zRes < Settings.HeightmapResolution; zRes++) {
                    for (var xRes = 0; xRes < Settings.HeightmapResolution; xRes++) {
                        ...
                        heightmap[zRes, xRes] = (GetNoise(xCoordinate, zCoordinate) / 2f) +0.5f;
                    }
                }
                Heightmap = heightmap;
            }
...
#region Chunk removal
        ...
        public void RemoveFromNeighborhood(TerrainChunk chunk) {
            if (Neighborhood.XDown == chunk)    Neighborhood.XDown = null;
            if (Neighborhood.XUp == chunk)      Neighborhood.XUp = null;
            if (Neighborhood.ZDown == chunk)    Neighborhood.ZDown = null;
            if (Neighborhood.ZUp == chunk)      Neighborhood.ZUp = null;
        }
#endregion
#region Neighborhood
